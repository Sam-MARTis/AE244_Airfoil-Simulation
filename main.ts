...
const showCirculationBoundaryOption = document.getElementById("showCirculationBoundary") as HTMLInputElement;
...
const CustomFunctionInputArea = document.getElementById("CustomFunctionInputArea") as HTMLInputElement;
CustomFunctionInputArea.disabled = true;

const ClVsAlphaBut = document.getElementById("ClVsAlpha") as HTMLButtonElement;
const ClVsAlphaResult = document.getElementById("ClVsAlphaResult") as HTMLButtonElement;
...
const initializeCamberFunction = () => {
  if (customFunctionMenu.checked) {
    ...
    try {
        const camberFunctionIntermediate = new Function("x", `return ${CustomFunctionInputArea.value};`) as (x: number) => number;
...
const performClVsAlpha = (minAlphaDegrees: number = -3, maxAlphaDegrees:number= 12, dAlphaDegrees = 0.1):[number[], number[]] => {
...
ClVsAlphaBut.addEventListener("click", (e:Event) => {
  e.preventDefault()

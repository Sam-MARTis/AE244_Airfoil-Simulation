...
const plotOptionMenu = document.getElementById("plotOptions") as HTMLSelectElement
...
const plotCamberSlope = (xStart:number, yStart: number, pointCount: number,  scaleFactor: number, lwidth: number, colour:string) => {
  ctx.clearRect(0, 0, canvas.width, canvas.height)
  const dx = chordLength/(pointCount-1);
  ctx.beginPath()
  ctx.lineWidth = lwidth
